...
public class View extends JFrame implements Observer {
    ...
    private void showGameBoard() {
        ...
        mainPanel = new JPanel() {
            private final Image boardImage = new ImageIcon("img/Board.png").getImage();
            ...
            protected void paintComponent(Graphics g) {
                ...
                // Vẽ bàn cờ
                g2d.drawImage(boardImage, -5, 0, (cellSize * 17) + 13, (cellSize * 17) + 13, null);

                // 1.5.1 drawPieceAtBarn()
                drawPieceAtBarn(cellSize, g2d);

                // 1.5.2 drawPiece()
                drawPiece(cellSize, g2d);

                // 1.5.3 drawHighlights()
        ...
        prepareDice();
        labelDice = new JLabel(iconDice[0]);
    ...
    /**
     * 1.2 notifyMove() - Model gọi sau khi thực hiện nước đi
     */
    ...
    public void updateMove() {
        ...
        SwingUtilities.invokeLater(() -> {
            if (mainPanel != null) {
                mainPanel.repaint();     // 1.4 repaint()
            }
        });
    }

    // 1.5 paintComponent() - Được gọi tự động sau repaint()
    // (đã implement trong anonymous class của mainPanel)
    ...
    public void drawPiece(int cellSize, Graphics2D g2d) {
        ...
        for (Player player : players) {
            for (Piece p : player.pieceList) {
                ...
                BoardCell cell = p.getBoardPosition() <= -2 
                    ? player.gridGoal.get(p.getGoalPosition())
                    : board.getGridNormal().get(p.getBoardPosition());

                drawingPiece(g2d, cell, player, cellSize, p.equals(selectedPiece));
            }
        }
    }
    ...
    // 1.6 Hiển thị trạng thái mới → Đã hoàn tất qua paintComponent()
    ...
    public void drawAllHighlights(Graphics2D g2d, int cellSize) {
        ...
        for (Piece piece : game.getMovablePieces(game.getDice().getResult())) {
            if (piece.getBoardPosition() < 0) continue;
            if (!game.canMove(piece)) continue;
    ...
    void prepareDice() {
        ...
        for (int i = 0; i < numberSide; i++) {
            ...
            iconDice[i] = new ImageIcon(newImg);
        }
    }
    ...
    public void updateSwitchTurn() {
        // Cập nhật giao diện lượt chơi nếu cần
    }
